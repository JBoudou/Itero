...
func NewPollNotification(evt events.Event) (ret *PollNotification) {
	...
	switch e := evt.(type) {
	case services.StartPollEvent:
		ret.Id = e.Poll
		ret.Action = PollNotifStart

	case services.NextRoundEvent:
		ret.Id = e.Poll
		ret.Action = PollNotifNext
		ret.Round = e.Round

	case services.ClosePollEvent:
		ret.Id = e.Poll
...
func (self *pollNotifRunner) filter(evt events.Event) bool {
	switch evt.(type) {
	case services.StartPollEvent, services.NextRoundEvent, services.ClosePollEvent, DeletePollEvent:
		return true
	}
	return false
}
